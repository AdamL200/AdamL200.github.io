...
console.log("working")
var hidTxt = document.getElementById('hidTxt')

var hidBtn = document.getElementById('hidBtn');
// Update the current slider value (each time you drag the slider handle)
hidBtn.onclick = function(){ func1()};
...
slider.oninput = function() {
    ...
    output.style.backgroundColor = "#"+randomColor2;
    if (this.value == 100){
        hidBtn.classList.remove('hidden');
    } else {
        hidBtn.classList.add('hidden');
    }
}
...
function func1() {
    console.log("1")
    if(hidTxt.classList.contains('hidden')) {
        hidTxt.classList.remove('hidden');
    }
};

var snkBtn = document.getElementById('snkBtn');
snkBtn.onclick = function() {displaysnake()}

function displaysnake(){
    snkBtn.classList.add('hidden')
